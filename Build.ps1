...
function Download-Files {
    ...
    $webClient.DownloadFile($editionUri, "$PSScriptRoot\Microsoft-Windows-EditionSpecific-EnterpriseG-Reconstruction-Package.ESD")
    cls 
}
...
if ($Type -in "Normal", "24H2", "Legacy") {
    ...
    Rename-Item -Path "sxs\Microsoft-Windows-EnterpriseG-ReconstructionEdition~31bf3856ad364e35~amd64~~10.0.22621.1.mum" -NewName "Microsoft-Windows-EnterpriseG-ReconstructionEdition~31bf3856ad364e35~amd64~~$Build.mum" -Force
    Rename-Item -Path "sxs\Microsoft-Windows-EnterpriseG-ReconstructionEdition~31bf3856ad364e35~amd64~~10.0.22621.1.cat" -NewName "Microsoft-Windows-EnterpriseG-ReconstructionEdition~31bf3856ad364e35~amd64~~$Build.cat" -Force

    (Get-Content "sxs\Microsoft-Windows-EnterpriseG-ReconstructionEdition~31bf3856ad364e35~amd64~~$Build.mum") -replace '10\.0\.22621\.1', $Build | Set-Content "sxs\Microsoft-Windows-EnterpriseG-ReconstructionEdition~31bf3856ad364e35~amd64~~$Build.mum" -Force
...
Copy-Item -Path mount\Windows\servicing\Editions\EnterpriseG-ReconstructionEdition.xml -Destination mount\Windows\EnterpriseG-Reconstruction.xml -ErrorAction SilentlyContinue

Write-Host
Write-Host "$([char]0x1b)[48;2;20;14;136m=== Setting SKU to Enterprise G"
dism /image:mount /apply-unattend:mount\Windows\EnterpriseG-Reconstruction.xml  
if ($Type -eq "24H2") {
    Dism /Image:mount /Set-Edition:EnterpriseG-Reconstruction /AcceptEula /ProductKey:FV469-WGNG4-YQP66-2B2HY-KD8YX  
} else {
    Dism /Image:mount /Set-Edition:EnterpriseG-Reconstruction /AcceptEula /ProductKey:YYVX9-NTFWV-6MDM3-9PT4T-4M68B  
}
$currentEdition = (dism /image:mount /get-currentedition | Out-String)

if ($currentEdition -match "EnterpriseG-Reconstruction") {
    Write-Host
    Write-Host "$([char]0x1b)[48;2;20;14;136m=== SKU successfully updated to EnterpriseG-Reconstruction"
} else {
    Write-Host
...
if ($Type -eq "24H2") {
    takeown /f "mount\Windows\System32\en-US\Licenses\_Default\EnterpriseG-Reconstruction\placeholder.rtf"   
    icacls "mount\Windows\System32\en-US\Licenses\_Default\EnterpriseG-Reconstruction\placeholder.rtf" /grant:r "$($env:USERNAME):(W)"  
    Copy-Item -Path "files\License\license.rtf" -Destination "mount\Windows\System32\en-US\Licenses\_Default\EnterpriseG-Reconstruction\placeholder.rtf" -Force  
    Copy-Item -Path "files\License\license.rtf" -Destination "mount\Windows\System32\en-US\Licenses\_Default\EnterpriseG-Reconstruction\license.rtf" -Force  
    Write-Host
}
else {
    $licensePath = "mount\Windows\System32\Licenses\neutral\_Default\EnterpriseG-Reconstruction"; if (-not (Test-Path -Path $licensePath -PathType Container)) { New-Item -Path $licensePath -ItemType Directory -Force }
    Copy-Item -Path "files\License\license.rtf" -Destination "mount\Windows\System32\Licenses\neutral\_Default\EnterpriseG-Reconstruction\license.rtf" -Force  
Write-Host
}
...
& "files\wimlib-imagex" info install.wim $ProIndex --image-property NAME="Windows $Windows Enterprise G" --image-property DESCRIPTION="Windows $Windows Enterprise G" --image-property FLAGS="EnterpriseG-Reconstruction" --image-property DISPLAYNAME="Windows $Windows Enterprise G" --image-property DISPLAYDESCRIPTION="Windows $Windows Enterprise G"  
Write-Host
